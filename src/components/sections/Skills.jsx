...
/* Inline monochrome SVG icons for each category */
const categoryIcons = [
  // Languages — code brackets
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M17.25 6.75 22.5 12l-5.25 5.25m-10.5 0L1.5 12l5.25-5.25m7.5-3-4.5 16.5" /></svg>,
  // Frameworks — cpu
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 w-full">
          {portfolioData.skills.map((category, catIndex) => (
            <motion.div
              ...
              className="bg-surface border border-border rounded-xl p-6 transition-all duration-300 hover:border-primary/20 group"
            >
              {/* Header */}
              <div className="flex items-center gap-3 mb-5">
                ...
                <h3 className="text-xs font-bold tracking-widest uppercase text-primary">{category.title}</h3>
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                {category.skills.map((skill, index) => (
                  <motion.span
                    ...
                    whileHover={{ y: -2 }}
                    className="px-3 py-1.5 bg-background border border-border rounded-lg text-xs font-medium text-primary-muted cursor-default transition-all duration-200 hover:border-primary/30 hover:text-primary"
                  >
                    {skill}
                  </motion.span>
                ))}
